...
1. How is the client sending data to the server? What protocol?
  The client is ending data to the server via TCP (SOCK_STREAM).

2. What data is the client sending to the server?
  The client is sending 5 messages from an array of messages one at a time.

Understanding the Server:
1. Explain the argument that the `run_acceptor` thread is passed as an argument.
  'run_acceptor' is passed a struct which has a running status variable 'run'
for the thread to decide whether to keep the thread running or to stop, a
'list_handle' to access the list of messages and a 'list_lock' to safely access
the list and count of messages in the list.
...
3. What does `main()` do with the received messages?
  main() counts the number of messages from the list of received mesaages to
check wheteher correct number of messages were received or not.

4. How are threads used in this sample code?
  For each client a new thread is spwned which then runs the connection between
server and the client, receiving the messages sent by the client.

5. How are sockets made non-blocking?
  Sockets are made non made non-blocking by using the fcntl() sys call with the
O_NONBLOCK flag

6. What sockets are made non-blocking?
  The server and all the client sockets are made non-blocking.

7. Why are these sockets made non-blocking? What purpose does it serve?
  The server socket is made non-blocking so that accept() is not blockinng and
busy waiting for a new client. The client sockets are made non-blocking so that
read() is not blocking and busy waiting for a new input from the client.
*/
#include <arpa/inet.h>
#include <errno.h>
#include <fcntl.h>
