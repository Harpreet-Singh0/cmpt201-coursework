...
int main() {
  ...
  if (PEM_read_PUBKEY(fd, &pubkey, NULL, NULL) == NULL) {
...
/*
    ...
    Returns:
        ...
        -1: Message does not match signature
*/
int verify(const char *message_path, const char *sign_path, EVP_PKEY *pubkey) {
#define MAX_FILE_SIZE 512
  ...
  if ((msglen = read_all_bytes(message_path, message, MAX_FILE_SIZE)) <= 0) {
    handle_error("error reading message");
  }

  if ((siglen = read_all_bytes(sign_path, signature, MAX_FILE_SIZE)) <= 0) {
    handle_error("error reading signature");
  }
  ...
  if (1 != EVP_DigestVerifyFinal(mdctx, signature, siglen)) {
    EVP_MD_CTX_free(mdctx);
    return -1;
  } else {
    EVP_MD_CTX_free(mdctx);
